...
This chapter produces the core deliverable of Stage-Gate Stage~2
(Scoping) --- the product specification and risk register that justify a
Go decision at Gate~2.
The central question and fidelity statement you produced in Chapter~3
define the \emph{goal}; the specification turns that goal into
measurable, testable requirements, and the risk register ranks the
unknowns the prototype must resolve before Gate~2 can be passed.
...
\begin{enumerate}
  ...
  \item separate requirements (the \emph{what}) from design decisions (the \emph{how});
  \item build and defend a risk-priority ranking that guides prototype focus; and
  \item identify the critical unknown in a set of open assumptions and justify which prototype experiment must be run first.
...
Without a principled ranking, you either chase the most visible problem rather than the most dangerous one, or try to mitigate everything simultaneously and mitigate nothing well.
...
Equation~\eqref{eq:ch4-risk} is fast to communicate but conflates two distinct ideas: the probability that a failure occurs and the team's ability to detect it before it reaches the user.
Section~\ref{sec:ch4-fmea} adds detection as a third factor.
...
These are the failure modes you must address before your Gate~2 submission.
...
\begin{itemize}
  ...
  \item \textbf{Step 3 — allocate the budget:}
        ...
        \begin{equation}
          \label{eq:ch4-budget-sum}
          t_{\text{firmware}} = 1\,\text{ms} + 2\,\text{ms} + 2\,\text{ms} = 5\,\text{ms}.
        \end{equation}
        The remaining $1{,}000 - 5 = 995\,\text{ms}$ is dwell-time margin before the next item enters the gate.
...
The team identifies three high-risk subsystems from the Chapter~3 scoping exercise: (A)~Sort Sensor, (B)~MQTT/Wi-Fi Integration, (C)~Divert Actuator.
...
\begin{enumerate}
  ...
  \item Assign $S$, $O$, and $D$ scores on the 1–10 scale with written justification for each S, O, and D value (nine justifications total: three failure modes × three dimensions).
  ...
  \item For this cross-check, use the $O$ score as $P_{\text{failure}}$ and the $S$ score as $C_{\text{consequence}}$, consistent with the worked example convention. Cross-check your top-ranked item using Equation~\eqref{eq:ch4-risk} and comment on whether the two rankings agree.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{(Applied — multi-step calculation)} A wearable ECG monitor has a motion-artifact failure mode with the following FMEA scores: Severity $S = 8$, Occurrence $O = 4$, Detection $D = 3$.
        \begin{enumerate}
          \item[(a)] Compute $R = P_{\text{failure}} \times C_{\text{consequence}}$ (Equation~\ref{eq:ch4-risk}) using $P_{\text{failure}} = O = 4$ and $C_{\text{consequence}} = S = 8$ as the cross-check convention. Does this failure mode qualify as high-risk ($R > 16$)?
          \item[(b)] Compute the RPN. The team then adds a motion-artifact filter that lowers $O$ from 4 to 2 while $S$ and $D$ remain unchanged. Recompute the RPN and state whether the mitigation moves the failure mode below the RPN threshold of 80.
          \item[(c)] Comment in one sentence on whether the $R$-score ranking (Part~a) and the RPN ranking (Part~b) agree.
        \end{enumerate}
...
You produce the core of \textbf{Milestone~1: the Proposal} --- together these three parts constitute your Gate~2 evidence package, the scoping deliverables that earn a Go decision to proceed with development.
...
\emph{`What will this thing do, how will you know it works, and what could kill the project?'} --- these three parts of the Milestone~1 Proposal answer that investor question directly.
...
A specification is a contract: every element — functional requirements, performance targets, constraints, interfaces, and acceptance criteria — must be measurable and testable, or it is not yet a requirement.
...
Chapter~5 takes this artifact as its starting point: the specification defines the required behaviour at each subsystem boundary, and the risk register flags which interfaces carry the highest-consequence unknowns.
Chapter~5 shows how to decompose the system into subsystems, define the interfaces between them, and produce the architecture diagram that makes integration testable.
