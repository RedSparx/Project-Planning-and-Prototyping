...
Chapter~3 established which subsystems must be prototyped and which can be trusted from prior work or supplier data.
That left open a critical gap: \emph{what, exactly, must those prototyped subsystems do}, and \emph{which failure modes deserve attention first}?
Without precise, measurable requirements, a team cannot declare success, and without a principled risk ranking, engineering effort disperses across the least-important problems.
This chapter addresses both gaps by showing how to convert vague feature wishes into testable specifications and how to rank risks using two complementary tools: a simple risk equation and an FMEA-style Risk Priority Number.
