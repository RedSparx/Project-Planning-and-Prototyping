...
\subsubsection{Lifecycle Obligations}
...
The saving from detecting a flaw at an earlier stage $s_1$ rather than a later
stage $s_2$ is:
\begin{equation}
  \Delta C(s_1, s_2) = C_0\,\bigl(10^{s_2} - 10^{s_1}\bigr).
  \label{eq:ch3-delta-cost}
\end{equation}
For example, catching a flaw at Design ($s_1=1$) rather than Production ($s_2=3$):
$\Delta C(1,3) = C_0\,(10^3 - 10^1) = 990\,C_0$.
...
\subsubsection{Flaw~A: Sort Sensor Field of View Too Narrow}
...
\subsubsection{Saving from Early Detection}
\begin{align}
  \Delta C &= C_{\text{change}}(3) - C_{\text{change}}(1) \notag \\
           &= \$50{,}000 - \$500 \notag \\
           &= \$49{,}500
...
\end{align}
Using Equation~\ref{eq:ch3-delta-cost} directly:
$\Delta C(1,3) = \$50\,(10^3 - 10^1) = \$50 \times 990 = \$49{,}500$.

\subsubsection{Flaw~B: MQTT Reconnection Loop Blocks Sensor Pipeline}
...
\medskip
\noindent Using Equation~\ref{eq:ch3-delta-cost} for Flaw~B:
$\Delta C(1,3) = \$50\,(10^3 - 10^1) = \$50 \times 990 = \$49{,}500$.
...
A custom motor-driver PCB has a layout error: the via drill size is incompatible
with the contract manufacturer's minimum drill specification.  The baseline fix
cost at the Concept stage is $C_0 = \$80$~CAD.

\begin{enumerate}
  \item Using $C(s) = C_0 \cdot 10^{\,s}$, compute the cost to fix this error at
        each of the four stages: Concept ($s=0$), Design ($s=1$), Build ($s=2$),
        and Production ($s=3$).
  \item Using Equation~\ref{eq:ch3-delta-cost}, compute $\Delta C(1,\,3)$ ---
        the saving from catching the flaw at Design rather than Production.
  \item Apply the decision flow in Figure~\ref{fig:ch3-selective-decision} to the
        motor-driver PCB as a subsystem. Is it ``novel or unprecedented''?
        ``High uncertainty in performance''? ``High consequence if it fails''?
        State a decision (Prototype or Specify-and-trust) with a one-sentence
        justification.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{(Conceptual)} State the three criteria used to place a subsystem
  in the ``Prototype it'' column of the classification table.
  Give one example of a real subsystem that satisfies all three criteria, and one
  that satisfies none.

  \item \textbf{(Conceptual)} The cost-of-change figure uses a logarithmic
  $y$-axis. Explain why a linear scale would be misleading for this data, and
  describe what the log-scale shape implies about where to invest prototype effort
  in a project timeline.

  \item \textbf{(Applied --- calculation)} The baseline fix cost for a firmware
  bug is $C_0 = \$120$~CAD.
  \begin{enumerate}
    \item Using $C(s) = C_0 \cdot 10^{\,s}$, compute the fix cost at all four
          stages.
    \item Using Equation~\ref{eq:ch3-delta-cost}, compute $\Delta C(1,\,3)$.
    \item A triage decision moves the fix from Production ($s=3$) back to Design
          ($s=1$). Express the saving as a multiple of $C_0$.
  \end{enumerate}

  \item \textbf{(Applied --- multi-step)} The baseline fix cost is $C_0 = \$60$~CAD.
  A defect is discovered at the Build stage ($s=2$).
  \begin{enumerate}
    \item Compute the cost to fix it at $s=2$.
    \item A redesign review triggers an earlier fix at $s=1$ (Design).
          Compute the cost at $s=1$.
    \item Compute $\Delta C(1,\,2)$ using Equation~\ref{eq:ch3-delta-cost}.
    \item How many ``baseline hours'' (at $C_0$ each) does the earlier trigger save?
  \end{enumerate}

  \item \textbf{(Applied --- classification)} Using the decision flow in
  Figure~\ref{fig:ch3-selective-decision}, classify each of the following
  subsystems for a wearable ECG monitor. For each, answer the three questions
  (novel? uncertain? high consequence?) and state the decision with a one-line
  justification.
  \begin{enumerate}
    \item Off-the-shelf Bluetooth SoC (Nordic nRF52840, standard reference design).
    \item Custom analog front-end for ECG signal acquisition.
    \item Standard 500~mAh LiPo battery cell.
    \item Custom dry electrode adhesive geometry (new material, no prior data).
    \item USB-C charging circuit (TI BQ25619 reference design, datasheet verified).
  \end{enumerate}
