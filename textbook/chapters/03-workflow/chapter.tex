...
and Gate~3 (proof of concept accepted).
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item State the fidelity level from \S3.5 and justify the choice in one
          sentence.
    \item Identify one risk that would justify moving to a higher fidelity if
          the initial result is inconclusive.
  \end{enumerate}
...
A firmware team is deciding how much prototype fidelity to invest in before
committing to a custom RTOS port. They estimate the following parameters:
$E_0 = 1$, $k = 3.0$.

\begin{enumerate}
  \item Using $E(f) = E_0 \cdot k^{\,f}$, compute the relative effort at each
        fidelity level $f = 1, 2, 3, 4, 5$. Organise your results in a table.
  \item Compute the ratio $E(5)/E(2)$. What does this number mean in
        practical terms?
  \item If the central question can be answered at $f = 2$, what percentage of
        the Level-5 effort is wasted by building to $f = 5$ instead?
\end{enumerate}
